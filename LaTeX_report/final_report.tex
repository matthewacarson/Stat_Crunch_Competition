...
\begin{equation}
\dfrac{1}{\text{Average viewers}_i} = 4.043631\times10^{-5} + 9.1545008\times10^{-6} \ast \text{Mean weekly stream hours}_i \label{eq:reciprocal_model_w_coef}
\end{equation}
...
\begin{equation}
\text{Average viewers}_i = \dfrac{1}{4.043631\times10^{-5} + 9.1545008\times10^{-6} \ast Mean\ weekly\ stream\ hours_{i}} \label{eq:back_transformed_reciprocal_eq}
\end{equation}
...
R-squared is rather weak (0.25), but the difference in watch time (mean weekly watch hours) does explain some of the differences observed between mature and non-mature accounts. The coefficient for \texttt{Mature} is -0.080855823. Exponentiating the coefficient ($\emph{e}^{-0.080856}$) results in 0.9223. This is the ratio of followers for mature accounts to non-mature accounts when the natural log of mean weekly watch hours is held constant. In other words, mature accounts have 7.7 (1- 0.9223) percent fewer followers than non-mature accounts when controlling for differences in watch time.
...
After running each model, \texttt{Followers} was the best variable to predict \texttt{Partnered} status.

\vspace{2mm}
\noindent \textbf{Variable(s)} \hfill \textbf{AIC}
\hspace{0.4em}
\hrule
\begin{itemize}
	\item \texttt{Followers}  \dotfill 237.57
