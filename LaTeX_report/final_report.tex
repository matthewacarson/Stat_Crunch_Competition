...
\begin{itemize}
	\item \texttt{`Peak viewers'}: 245.79
	\item \texttt{`Average viewers'}: 246.42
	\item \texttt{`Mature'}: 246.22
	\item \texttt{`Mean weekly watch hours'}: 245.77
	\item \texttt{`Mean weekly stream hours'}: 246.36
\end{itemize}
...
\begin{itemize}
\begin{align}
	\item \texttt{`Followers + Peak viewers'} &: 238.8016
	\item \texttt{`Followers + Average viewers'} &: 237.6662
	\item \texttt{`Followers + Mature'} &: 239.0717
	\item \texttt{`Followers + Mean weekly watch hours'} &: 239.0692
	\item \texttt{`Followers + Mean weekly stream hours'} &: 238.3238
\end{align}
\end{itemize}
...
\begin{equation}
\begin{aligned}
	ln(\frac{p_{i}}{1-p_{i}}) & = logit(Partnered_{i})  = \beta_{0} + \beta_{1} \ast Followers_{i} \\
	\frac{p_{i}}{1-p_{i}} & = odds
\end{aligned}
\end{equation}
where \emph{i} is a Twitch account; \texttt{`Partnered'} is a binary variable indicating whether the respective account has attained partnered status or not; \texttt{`Followers'} is the numer of followers that an account has; and \emph{p} is the probability of a "success."
